...
let s = 0,
  len = 0;
for (const odd of Object.values(game.odds)) s += odd;
console.log(`avarage is: ${s / Object.values(game.odds).length}`);

// 3
for (const [team, odd] of Object.entries(game.odds)) {
  let teamStr = team == "x" ? "draw" : `victory ${game[team]}`;
  let str = `Odd of ${teamStr}: ${odd}`;
  console.log(str);
}
...
for (const str of strArr) {
  let [first, second] = str.trim().split("_");
  // second = [second[0].toUpperCase(), ...second];
